...
function calculateScoresForDay(dayLimit) {
  ...
  if (dayLimit === "Day 1") {
    if (
      championData.day1_results &&
      championData.day1_results.overall_standings
    ) {
    ...
    if (
      championData.day2_results &&
      championData.day2_results.overall_standings
    ) {
...
function renderPodiumWithScores(scores) {
  ...
  const ranks = championData.teams
    .map((t) => ({
      ...t,
  ...
  container.querySelectorAll(".podium-block").forEach((block) => {
    block.addEventListener("click", () =>
      showScoreBreakdown(block.dataset.team),
    );
  });
}
...
function buildTimeline() {
  ...
  container.innerHTML = day.activities
    .map(
      (act, index) => `
...
function buildScoreboard() {
  ...
  if (activeResultsFilter !== "all") {
    ...
    events = events.filter((evt) => {
      ...
      if (filter === "minor")
        return (
          catName.includes("minor") ||
          catName.includes("larong") ||
          catName.includes("traditional")
        );
      if (filter === "musical") return catName.includes("musical");
      if (filter === "e-sports")
        return catName.includes("esports") || catName.includes("e-sports");
  ...
  if (events.length === 0) {
    container.innerHTML =
      '<div style="text-align:center; padding: 2rem; grid-column: 1/-1;">No events found for this category</div>';
    return;
  }
  ...
  container.innerHTML = events
    .map((evt) => {
      ...
      const winnerColor =
        (winner && getTeamColor(winner.team)) || "var(--text-muted)";
...
function showRoster(teamCode) {
  ...
  html += renderCategory(
    "Major Activities Team Members",
    team.roster.major_activities,
  );
  html += renderCategory(
    "Minor Activities Team Members",
    team.roster.minor_activities,
  );
...
function showScoreBreakdown(teamCode) {
  ...
  const day1Score =
    championData.day1_results.overall_standings.find(
      (s) => s.abbreviation === teamAbbrev,
    )?.total_points || 0;

  const day2Cumulative =
    championData.day2_results.overall_standings.find(
      (s) => s.abbreviation === teamAbbrev,
    )?.total_points || 0;
  const day2Score = day2Cumulative - day1Score;

  const day3Score =
    championData.day3_results.overall_standings.find(
      (s) => s.abbreviation === teamAbbrev,
    )?.total_points || 0;
