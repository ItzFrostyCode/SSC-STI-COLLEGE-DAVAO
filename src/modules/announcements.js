  ...
  try {
    ...
    const fetchUrl =
      typeof window !== "undefined" && window.location.protocol === "file:"
        ...
        : `data/announcements.json?t=${timestamp}`;
    ...
    if (data) {
      ...
      if (typeof Fuse !== "undefined") {
        ...
        console.warn(
          "Fuse.js not loaded, search will fall back to basic matching",
        );
      }
...
function renderPinned() {
  ...
  let pinnedItem = allAnnouncements.find(
    (item) => item.id === PINNED_ANNOUNCEMENT_ID,
  );
...
function renderFeed() {
  ...
  if (filtered.length === 0) {
    feed.innerHTML = "";
    const emptyType =
      searchQuery || currentFilters.category !== "all" || year || month || day
        ? "no-results"
        : "no-announcements";
...
function loadMorePosts() {
  ...
  const endIndex = Math.min(
    currentlyDisplayed + postsPerLoad,
...
function createFacebookCard(post, postIndex) {
  ...
  const shouldUseLargeText =
    mediaCount === 0 && !post.sharedPost && content.length < 80;
...
function getMediaForSharedPost(sharedPost) {
  ...
  if (sharedPost.gallery && Array.isArray(sharedPost.gallery)) {
    ...
    mediaItems = [
      { type: detectMediaType(sharedPost.image), url: sharedPost.image },
    ];
  }
...
function setupListeners() {
  ...
  if (dateFilterTrigger && dateFilterMenu) {
    ...
    if (applyBtn) {
      applyBtn.addEventListener("click", () => {
        ...
        dateFilterLabel.textContent =
          parts.length > 0 ? parts.join(" ") : "Select Date";
    ...
    document.addEventListener("click", (e) => {
      if (
        !dateFilterTrigger.contains(e.target) &&
        !dateFilterMenu.contains(e.target)
      ) {
        dateFilterTrigger.classList.remove("active");
        dateFilterMenu.classList.remove("show");
      }
...
function createEmptyState(type = "no-results") {
  ...
  const emptyStateConfigs = {
    ...
    "no-announcements": {
      ...
      message:
        "There are currently no announcements available. Check back soon for updates!",
      actions: [],
    },
  };
