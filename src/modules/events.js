...
function updateCounts() {
  ...
  if (document.getElementById("count-1st"))
    document.getElementById("count-1st").textContent = counts["1st"];
  if (document.getElementById("count-2nd"))
    document.getElementById("count-2nd").textContent = counts["2nd"];

  if (document.getElementById("count-all-mobile"))
    document.getElementById("count-all-mobile").textContent = counts.all;
  if (document.getElementById("count-1st-mobile"))
    document.getElementById("count-1st-mobile").textContent = counts["1st"];
  if (document.getElementById("count-2nd-mobile"))
    document.getElementById("count-2nd-mobile").textContent = counts["2nd"];
}
  ...
  if (filtered.length === 0) {
    ...
    if (emptyState) {
      ...
      emptyState.querySelector("p").textContent =
        `There are no events ${filterText}.`;
    }
    return;
  }
...
function getAvatarUrl(organizer) {
  const avatarMap = {
    "Cherry Jane Villasencio - PIO":
      "assets/images/officers/cherry-jane-villasencio.webp",
    "Cherry Jane Villasencio":
      "assets/images/officers/cherry-jane-villasencio.webp",
    "SSC Events Committee": "assets/images/homepage/ssc-logo-removebg.webp",
  };
  return (
    avatarMap[organizer] || "assets/images/homepage/ssc-logo-removebg.webp"
  );
}
...
function createEventCard(event) {
  const firstImage =
    ...
    event.image ||
    "assets/images/homepage/ssc-logo.webp";
...
function updateLightboxImage() {
  ...
  if (caption)
    caption.textContent = `Image ${currentLightboxIndex + 1} of ${lightboxImages.length}`;
}
...
function setupListeners() {
  ...
  document.querySelectorAll(".desktop-only .filter-btn").forEach((btn) => {
    ...
    newBtn.addEventListener("click", () => {
      document
        .querySelectorAll(".desktop-only .filter-btn")
  ...
  if (sortBtn) {
    ...
    newSort.addEventListener("click", () => {
      ...
      if (svg)
        svg.style.transform =
          currentSort === "newest" ? "rotate(0)" : "rotate(180deg)";
      renderEvents();
    });
  }
